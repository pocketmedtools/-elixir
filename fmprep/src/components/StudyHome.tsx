...
export default function StudyHome({ onGo }: { onGo: (view: StudyView) => void }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-3 py-5 md:px-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Family Medicine exam preparation</h1>
      ...
      <button
        ...
        className="mt-4 flex w-full items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-left text-sm text-slate-500 shadow-sm"
      >
        <Search className="h-4 w-4" aria-hidden /> Search notes, answers, cases and your documents
      </button>

      {(last || streak > 0 || readCount > 0) && (
        <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm">
          {streak > 0 && (
          ...
          {last && (
            <button
              ...
              className="ml-auto flex items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-bold text-white"
            >
              <BookMarked className="h-3.5 w-3.5" aria-hidden /> Continue: {last.topic.title.slice(0, 34)}
        ...
        {SECTIONS.map((s) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => onGo(s.view)}
              className={`flex items-start gap-3 rounded-xl border p-4 text-left shadow-sm transition hover:shadow ${s.tint}`}
            >
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs leading-snug text-slate-700">{s.body}</span>
              </span>
            </button>
          );
        })}
      ...
      {!ready && (
        <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="font-bold text-slate-900">Make the whole library available offline</p>
          ...
          {downloading ? (
            ...
            <button
              ...
              className="mt-3 flex items-center gap-2 rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white"
            >
      ...
      <section className="mt-8">
        <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500">Jump to a subject</h2>
        {SUBJECT_GROUPS.map((group) => (
          <div key={group.heading} className="mt-3">
            <p className="text-sm font-bold text-slate-800">{group.heading}</p>
