...
const THEMES = [
  { name: "light", washL: 96, hlL: 84, hlS: 88, paper: "#F7F6F3", raised: "#FFFFFF", ink: "#11182A" },
  { name: "dark", washL: 14, hlL: 30, hlS: 54, paper: "#0D1220", raised: "#141B2B", ink: "#E9EBF0" },
];
...
for (const t of THEMES) {
  ...
  for (const [id, c] of Object.entries(SUBJECT_COLOR)) {
    const h = c.h;
    const acc = hsl(h, 58, t.name === "light" ? c.l : c.d);
    const wash = hsl(h, t.name === "light" ? 62 : 36, t.washL);
    const hl = hsl(h, t.hlS, t.hlL);
    const checks = [
      ...
      [`highlighted text (${id})`, ratio(ink, hl), 4.5],
    ];
